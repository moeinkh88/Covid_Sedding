...
# IA=X0par[i,1]
# P=X0par[i,2]
S0=665188;E0=0;IA0=100;IS0=17;R0=0;R10=0;P0=100;D0=0;N0=S0+E0+IA0+IS0+R0
...
par=[Λ,μ,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σS,σA,γS,γA,ηS,ηA]
...
function  F(dx, x, par, t)
    ...
    Λ,μ,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σS,σA,γS,γA,ηS,ηA=par
    ...
    dx[3]= (1-δ)*ω*E - (μ+σA)*IA - γA*IA
    dx[4]= δ*ω*E - (μ + σS)*IS - γS*IS
    dx[5]=γS*IS + γA*IA - μ*R
    dx[6]=γS*IS - μ*R1
    dx[7]=ηA*IA + ηS*IS - μp*P
    dx[8]=σA*IA + σS*IS - μ*D
    dx[9]=Λ*N - σA*IA - σS*IS - μ*N
    return nothing
...
@model function fitprob(data,prob)
    ...
    # Simulate model.
	p=[Λ,μ,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σS,σA,γS,γA,ηS,ηA]
	N0=S0+E0+IA0+IS0+R0
...
display(chain)
##
posterior_samples = sample(chain[[:S0,:μ,:Λ,:μp,:ϕ1,:ϕ2,:β1,:β2,:δ,:ψ,:ω,:σS,:σA,:γS,:γA,:ηS,:ηA, :E0, :IA0, :P0]], Nch; replace=false)
...
for i in 1:Nch
	...
	μ,Λ,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σS,σA,γS,γA,ηS,ηA = pp[2:17]
	p = [Λ,μ,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σS,σA,γS,γA,ηS,ηA]
	S0=pp[1]
	E0=pp[18]
	IA0=pp[19]
