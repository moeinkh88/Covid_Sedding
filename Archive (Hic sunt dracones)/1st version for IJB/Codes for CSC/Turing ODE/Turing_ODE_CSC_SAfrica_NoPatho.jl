...
# IA=X0par[i,1]
# P=X0par[i,2]
S0=665188;E0=0;IA0=100;IS0=17;R0=0;D0=0;N0=S0+E0+IA0+IS0+R0
x0=[S0,E0,IA0,IS0,R0,D0,N0] # initial conditons S0,E0,IA0,IS0,R0,P0,D0,N0
...
par=[Λ,μ,ϕ2,β2,δ,ψ,ω,σ,γS,γA,ηS,ηA]
...
@model function fitprob(data,prob)
    ...
    σ ~ InverseGamma(2, 3)
	S0 ~ truncated(Normal(500,100000); lower=1000, upper=200000)
    ...
    # Simulate model.
	p=[Λ,μ,ϕ2,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
	N0=S0+E0+IA0+IS0+R0
...
display(chain)
##
posterior_samples = sample(chain[[:S0,:ϕ2,:β2,:δ,:ψ,:ω,:σ2,:γS,:γA,:ηS,:ηA, :IA0]], Nch; replace=false)
...
for i in 1:Nch
	...
	ϕ2,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA = pp[2:11]
	p = [Λ,μ,ϕ2,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
	S0=pp[1]
	IA0=pp[12]
	# P0=pp[16]
