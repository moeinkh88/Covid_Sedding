...
par=[Λ,μ,μp,β1,β2,δ,ψ,ω,σ,γS,γA,ηS,ηA]
...
function  F(dx, x, par, t)
    ...
    Λ,μ,μp,β1,β2,δ,ψ,ω,σ,γS,γA,ηS,ηA=par
    S,E,IA,IS,R,P,D,N=x

    dx[1]= Λ*N - β1*S*P - β2*S*(IA + IS) + ψ*E - µ*S
    dx[2]= β1*S*P + β2*S*(IA+IS) - ψ*E - μ*E - ω*E
...
@model function fitprob(data,prob)
    ...
    # Simulate model.
	p=[Λ,μ,μp,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
	N0=S0+E0+IA0+IS0+R0
...
display(chain)
##
posterior_samples = sample(chain[[:S0,:μp,:β1,:β2,:δ,:ψ,:ω,:σ2,:γS,:γA,:ηS,:ηA, :IA0, :P0]], Nch; replace=false)
...
for i in 1:Nch
	...
	μp,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA = pp[2:12]
	p = [Λ,μ,μp,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
	S0=pp[1]
	IA0=pp[13]
	P0=pp[14]
