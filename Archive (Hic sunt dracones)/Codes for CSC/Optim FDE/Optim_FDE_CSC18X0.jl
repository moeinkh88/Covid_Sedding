...
p_lo_1=vcat(.5*ones(7),1e-5*ones(4),1e4,zeros(3)) #lower bound
p_up_1=vcat(ones(7),222,5*ones(3),2e6,30*ones(3)) # upper bound
# p_vec_1=vcat(.99999*ones(7),Λ,ϕ2,δ,ηA,γA,ω)
p_vec_1=vcat(0.9999999998004621, 0.9999999964668235, 0.9265172839266816, 0.9835508220131483, 0.999999999046695, 0.9999999907091582,1, 1.221807552538849e-5, 3.768795628177853, 0.592964835141523, 0.5983303276384108, 1.765239357851899e6,0.0001,0.0001,0.0001)

print("Λ,ϕ2,δ,ηA")

Res=optimize(loss,p_lo_1,p_up_1,p_vec_1,Fminbox(BFGS()),# Broyden–Fletcher–Goldfarb–Shanno algorithm
