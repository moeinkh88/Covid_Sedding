...
p_lo_1=vcat(.5*ones(8),1e-5*ones(5)) #lower bound
p_up_1=vcat(ones(8),3*ones(5)) # upper bound
p_vec_1=vcat(.999999*ones(8),ϕ2,δ,ψ,γA, ηA )

show("we are fitting orders and ϕ2,δ,ψ,γA, ηA , in this code: Optim_FDE_CSC_Birth3.jl")

Res=optimize(loss,p_lo_1,p_up_1,p_vec_1,Fminbox(
LBFGS()),# Broyden–Fletcher–Goldfarb–Shanno algorithm
