...
Λ=0.18 # recruitment rate in the population
μ=9.3e-3 # natural human death rate
...
@model function fitprob(data,prob)
	...
	μ ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
    Λ ~ truncated(Normal(1, 300); lower=ϵ, upper=300)
	μp ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ϕ1 ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ϕ2 ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	β1 ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	β2 ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	δ ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ψ ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ω ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	σ2 ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	γS ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	γA ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ηS ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
	ηA ~ truncated(Normal(0, 2); lower=ϵ, upper=2)
