...
S0= 25060.216457227714;E0=35.96791914222955;IA0=167.56655046390654;IS0=17;R0=0;P0=10.718478095775652;D0=0;N0=S0+E0+IA0+IS0+R0
...
par =[α1,α2,μ1,μ2,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
...
function loss(args)
	...
	Order[1:6]=args[1:6]
	Order[8]=args[7]
	E0,α2,μ2,ϕ2, ω, δ, γA, ηA =args[8:15]
	# E0, IA0, P0 =args[8:10]
...
p_lo_1=vcat(.5*ones(7),zeros(8)) #lower bound
p_up_1=vcat(ones(7),300,ones(7)) # upper bound
p_vec_1=vcat(.9999*ones(7),0,0,0,ϕ2, ω, δ, γA, ηA)
# p_lo_1=vcat(.5*ones(9),1e-3*ones(4)) #lower bound
# p_up_1=vcat(ones(9),ones(4)) # upper bound
# p_vec_1=vcat(.99999*ones(9), ϕ2,δ, ω, γA)


show("we are fitting orders and α2,μ2")
