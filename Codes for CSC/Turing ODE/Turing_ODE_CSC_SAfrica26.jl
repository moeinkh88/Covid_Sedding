# initial fit (unbounded) with birth rate and density dependent population
using Statistics
...
par=[α1,α2,μ1,μ2,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σ,γS,γA,ηS,ηA]
...
function  F(dx, x, par, t)
    ...
    dx[1]= Λ*N - β1*S*P/(1+ϕ1*P) - β2*S*(IA + IS)/(1+ϕ2*(IA+IS)) + ψ*E - μ*S
...
@model function fitprob(data,prob)
	...
	α2 ~ truncated(Normal(0, 1e-8); lower=0, upper=1e-7)
    μ2 ~ truncated(Normal(0, 1e-8); lower=0, upper=1e-7)
...
display(chain)
##
posterior_samples = sample(chain[[:S0,:α2,:μ2,:μp,:ϕ1,:ϕ2,:β1,:β2,:δ,:ψ,:ω,:σ2,:γS,:γA,:ηS,:ηA, :E0, :IA0, :P0]], Nch; replace=false)
...
for i in 1:Nch
	...
	α2,μ2,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA = pp[2:16]
	p = [α1,α2,μ1,μ2,μp,ϕ1,ϕ2,β1,β2,δ,ψ,ω,σ2,γS,γA,ηS,ηA]
	S0=pp[1]
	E0=pp[17]
	IA0=pp[18]
